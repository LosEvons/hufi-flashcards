...
router.get('/decks/:deckId/cards', (req, res) => {
  ...
  try {
    ...
    const cards = all(
      'SELECT id, finnish, hungarian, created_at FROM Card WHERE deck_id = ? ORDER BY created_at DESC',
      [deckId]
    );
...
router.put('/cards/:cardId', (req, res) => {
  ...
  try {
    ...
    const newFinnish = finnish ?? card.finnish;
    const newHungarian = hungarian ?? card.hungarian;
